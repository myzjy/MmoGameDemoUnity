...
namespace TBTK
{
    public class Unit : TBMonoItem
    {
        ...
        {
            return stats.hp * GetFullHPMul() + GetFullHPMod();
        }

        public float GetFullHPMul()
        {
            return activeEffectMul.stats.hp * PerkManager.GetUnitMulHP(prefabID);
        }

        public float GetFullHPMod()
        {
            return activeEffectMod.stats.hp + PerkManager.GetUnitModHP(prefabID);
        }

        public float GetFullAP()
        {
            return stats.ap * GetFullAPMul() + GetFullAPMod();
        }

        public float GetFullAPMul()
        {
            return activeEffectMul.stats.ap * PerkManager.GetUnitMulAP(prefabID);
        }

        public float GetFullAPMod()
        {
            return activeEffectMod.stats.ap + PerkManager.GetUnitModAP(prefabID);
        }

        public float GetHPRegen()
        {
            return stats.hpRegen * GetHPRegenMul() + GetHPRegenMod();
        }

        public float GetHPRegenMul()
        {
            return activeEffectMul.stats.hpRegen * PerkManager.GetUnitMulHPRegen(prefabID);
        }

        public float GetHPRegenMod()
        {
            return activeEffectMod.stats.hpRegen + PerkManager.GetUnitModHPRegen(prefabID);
        }

        public float GetAPRegen()
        {
            return stats.apRegen * GetAPRegenMul() + GetAPRegenMod();
        }

        public float GetAPRegenMul()
        {
            return activeEffectMul.stats.apRegen * PerkManager.GetUnitMulAPRegen(prefabID);
        }

        public float GetAPRegenMod()
        {
            return activeEffectMod.stats.apRegen + PerkManager.GetUnitModAPRegen(prefabID);
        }


        public int GetMoveLimit()
        {
            return (int)(stats.moveLimit * GetMoveLimitMul()) + (int)GetMoveLimitMod();
        }

        public float GetMoveLimitMul()
        {
            return activeEffectMul.stats.moveLimit * PerkManager.GetUnitMulMoveLim(prefabID);
        }

        public float GetMoveLimitMod()
        {
            return activeEffectMod.stats.moveLimit + PerkManager.GetUnitModMoveLim(prefabID);
        }
        ...
        public float GetAttackLimitMul()
        {
            return activeEffectMul.stats.moveLimit * PerkManager.GetUnitMulAttackLim(prefabID);
        }

        public float GetAttackLimitMod()
        {
            return activeEffectMod.stats.moveLimit + PerkManager.GetUnitModAttackLim(prefabID);
        }

        public int GetAttackRemain()
        {
        ...
        public float GetCounterLimitMul()
        {
            return activeEffectMul.stats.counterLimit * PerkManager.GetUnitMulCounterLim(prefabID);
        }

        public float GetCounterLimitMod()
        {
        ...
        {
            return (int)(stats.abilityLimit * GetAbilityLimitMul() + GetAbilityLimitMod());
        }

        public float GetAbilityLimitMul()
        {
            return activeEffectMul.stats.abilityLimit * PerkManager.GetUnitMulAbilityLim(prefabID);
        }

        public float GetAbilityLimitMod()
        {
            return activeEffectMod.stats.abilityLimit + PerkManager.GetUnitModAbilityLim(prefabID);
        }
        ...
        public void NewTurn(bool restoreFullHP = false)
        {
            //restoreFullHP is used when the game start
        ...
        */
        ...
        public bool InDistance(Node tgtNode, int tgtDist)
        {
            return GridManager.GetDistance(node, tgtNode) <= tgtDist;
        }

        public float GetAuraAttackMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.attack;
            }

            return value;
        }

        public float GetAuraDefenseMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.defense;
            }

            return value;
        }

        public float GetAuraHitMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.hit;
            }

            return value;
        }

        public float GetAuraDodgeMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.dodge;
            }

            return value;
        }

        public float GetAuraDmgHPMinMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.dmgHPMin;
            }

            return value;
        }

        public float GetAuraDmgHPMaxMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.dmgHPMax;
            }

            return value;
        }

        public float GetAuraDmgAPMinMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.dmgAPMin;
            }

            return value;
        }

        public float GetAuraDmgAPMaxMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.dmgAPMax;
            }

            return value;
        }

        public float GetAuraCritChanceMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.critChance;
            }

            return value;
        }

        public float GetAuraCritReducMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.critReduc;
            }

            return value;
        }

        public float GetAuraCritMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.critMultiplier;
            }

            return value;
        }

        public float GetAuraCDmgMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.cDmgMultip;
            }

            return value;
        }

        public float GetAuraCHitPenaltyMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.cHitPenalty;
            }

            return value;
        }

        public float GetAuraCCritPenaltyMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.cCritPenalty;
            }

            return value;
        }

        public float GetAuraODmgMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.oDmgMultip;
            }

            return value;
        }

        public float GetAuraOHitPenaltyMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.oHitPenalty;
            }

            return value;
        }

        public float GetAuraOCritPenaltyMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.oCritPenalty;
            }

            return value;
        }

        public float GetAuraAttackRangeMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.attackRange;
            }

            return value;
        }

        public float GetAuraAttackRangeMinMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.attackRangeMin;
            }

            return value;
        }

        public float GetAuraMoveRangeMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.moveRange;
            }

            return value;
        }

        public float GetAuraTurnPriorityMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.turnPriority;
            }

            return value;
        }

        public float GetAuraSightMul(Node tgtNode, float value = 1)
        {
            for (int i = 0; i < auraMulList.Count; i++)
            {
                if (InDistance(tgtNode, auraMulList[i].range)) value *= auraMulList[i].stats.sight;
            }
        ...
        public float GetAuraAttackMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.attack;
            }

            return value;
        }

        public float GetAuraDefenseMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.defense;
            }

            return value;
        }

        public float GetAuraHitMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.hit;
            }

            return value;
        }

        public float GetAuraDodgeMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.dodge;
            }

            return value;
        }

        public float GetAuraDmgHPMinMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.dmgHPMin;
            }

            return value;
        }

        public float GetAuraDmgHPMaxMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.dmgHPMax;
            }

            return value;
        }

        public float GetAuraDmgAPMinMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.dmgAPMin;
            }

            return value;
        }

        public float GetAuraDmgAPMaxMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.dmgAPMax;
            }

            return value;
        }

        public float GetAuraCritChanceMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.critChance;
            }

            return value;
        }

        public float GetAuraCritReducMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.critReduc;
            }

            return value;
        }

        public float GetAuraCritMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.critMultiplier;
            }

            return value;
        }

        public float GetAuraCDmgMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.cDmgMultip;
            }

            return value;
        }

        public float GetAuraCHitPenaltyMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.cHitPenalty;
            }

            return value;
        }

        public float GetAuraCCritPenaltyMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.cCritPenalty;
            }

            return value;
        }

        public float GetAuraODmgMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.oDmgMultip;
            }

            return value;
        }

        public float GetAuraOHitPenaltyMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.oHitPenalty;
            }

            return value;
        }

        public float GetAuraOCritPenaltyMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.oCritPenalty;
            }

            return value;
        }

        public float GetAuraAttackRangeMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.attackRange;
            }

            return value;
        }

        public float GetAuraAttackRangeMinMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.attackRangeMin;
            }

            return value;
        }

        public float GetAuraMoveRangeMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.moveRange;
            }

            return value;
        }

        public float GetAuraTurnPriorityMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.turnPriority;
            }

            return value;
        }

        public float GetAuraSightMod(Node tgtNode, float value = 0)
        {
            for (int i = 0; i < auraModList.Count; i++)
            {
                if (InDistance(tgtNode, auraModList[i].range)) value += auraModList[i].stats.sight;
            }
        ...
        {
            return stats.attack * GetAttackMul() + GetAttackMod();
        }

        public float GetAttackMul()
        {
            return activeEffectMul.stats.attack * PerkManager.GetUnitMulAttack(prefabID) *
                   UnitManager.GetAuraAttackMul(this, node);
        }

        public float GetAttackMod()
        {
            return activeEffectMod.stats.attack + PerkManager.GetUnitModAttack(prefabID) +
                   UnitManager.GetAuraAttackMod(this, node);
        }

        public float GetDefense()
        {
            return stats.defense * GetDefenseMul() + GetDefenseMod();
        }

        public float GetDefenseMul()
        {
            return activeEffectMul.stats.defense * PerkManager.GetUnitMulDefense(prefabID) *
                   UnitManager.GetAuraDefenseMul(this, node);
        }

        public float GetDefenseMod()
        {
            return activeEffectMod.stats.defense + PerkManager.GetUnitModDefense(prefabID) +
                   UnitManager.GetAuraDefenseMod(this, node);
        }

        public float GetHit()
        {
            return stats.hit * GetHitMul() + GetHitMod();
        }

        public float GetHitMul()
        {
            return activeEffectMul.stats.hit * PerkManager.GetUnitMulHit(prefabID) *
                   UnitManager.GetAuraHitMul(this, node);
        }

        public float GetHitMod()
        {
            return activeEffectMod.stats.hit + PerkManager.GetUnitModHit(prefabID) +
                   UnitManager.GetAuraHitMod(this, node);
        }

        public float GetDodge()
        {
            return stats.dodge * GetMulDodge() + GetModDodge();
        }

        public float GetMulDodge()
        {
            return activeEffectMul.stats.dodge * PerkManager.GetUnitMulDodge(prefabID) *
                   UnitManager.GetAuraDodgeMul(this, node);
        }

        public float GetModDodge()
        {
            return activeEffectMod.stats.dodge + PerkManager.GetUnitModDodge(prefabID) +
                   UnitManager.GetAuraDodgeMod(this, node);
        }

        public float GetDmgHPMin()
        {
            return stats.dmgHPMin * GetMulDmgHPMin() + GetModDmgHPMin();
        }

        public float GetMulDmgHPMin()
        {
            return activeEffectMul.stats.dmgHPMin * PerkManager.GetUnitMulDmgHPMin(prefabID) *
                   UnitManager.GetAuraDmgHPMinMul(this, node);
        }

        public float GetModDmgHPMin()
        {
            return activeEffectMod.stats.dmgHPMin + PerkManager.GetUnitModDmgHPMin(prefabID) +
                   UnitManager.GetAuraDmgHPMinMod(this, node);
        }

        public float GetDmgHPMax()
        {
            return stats.dmgHPMax * GetMulDmgHPMax() + GetModDmgHPMax();
        }

        public float GetMulDmgHPMax()
        {
            return activeEffectMul.stats.dmgHPMax * PerkManager.GetUnitMulDmgHPMax(prefabID) *
                   UnitManager.GetAuraDmgHPMaxMul(this, node);
        }

        public float GetModDmgHPMax()
        {
            return activeEffectMod.stats.dmgHPMax + PerkManager.GetUnitModDmgHPMax(prefabID) +
                   UnitManager.GetAuraDmgHPMaxMod(this, node);
        }

        public float GetDmgAPMin()
        {
            return stats.dmgAPMin * GetMulDmgAPMin() + GetModDmgAPMin();
        }

        public float GetMulDmgAPMin()
        {
            return activeEffectMul.stats.dmgAPMin * PerkManager.GetUnitMulDmgAPMin(prefabID) *
                   UnitManager.GetAuraDmgAPMinMul(this, node);
        }

        public float GetModDmgAPMin()
        {
            return activeEffectMod.stats.dmgAPMin + PerkManager.GetUnitModDmgAPMin(prefabID) +
                   UnitManager.GetAuraDmgAPMinMod(this, node);
        }

        public float GetDmgAPMax()
        {
            return stats.dmgAPMax * GetMulDmgAPMax() + GetModDmgAPMax();
        }

        public float GetMulDmgAPMax()
        {
            return activeEffectMul.stats.dmgAPMax * PerkManager.GetUnitMulDmgAPMax(prefabID) *
                   UnitManager.GetAuraDmgAPMaxMul(this, node);
        }

        public float GetModDmgAPMax()
        {
            return activeEffectMod.stats.dmgAPMax + PerkManager.GetUnitModDmgAPMax(prefabID) +
                   UnitManager.GetAuraDmgAPMaxMod(this, node);
        }

        public float GetCritChance()
        {
            return stats.critChance * GetMulCritChance() + GetModCritChance();
        }

        public float GetMulCritChance()
        {
            return activeEffectMul.stats.critChance * PerkManager.GetUnitMulCritC(prefabID) *
                   UnitManager.GetAuraCritChanceMul(this, node);
        }

        public float GetModCritChance()
        {
            return activeEffectMod.stats.critChance + PerkManager.GetUnitModCritC(prefabID) +
                   UnitManager.GetAuraCritChanceMod(this, node);
        }

        public float GetCritReduc()
        {
            return stats.critReduc * GetMulCritReduc() + GetModCritReduc();
        }

        public float GetMulCritReduc()
        {
            return activeEffectMul.stats.critReduc * PerkManager.GetUnitMulCritR(prefabID) *
                   UnitManager.GetAuraCritReducMul(this, node);
        }

        public float GetModCritReduc()
        {
            return activeEffectMod.stats.critReduc + PerkManager.GetUnitModCritR(prefabID) +
                   UnitManager.GetAuraCritReducMod(this, node);
        }

        public float GetCritMultiplier()
        {
            return stats.critMultiplier * GetMulCritMul() + GetModCritMul();
        }

        public float GetMulCritMul()
        {
            return activeEffectMul.stats.critMultiplier * PerkManager.GetUnitMulCritM(prefabID) *
                   UnitManager.GetAuraCritMul(this, node);
        }

        public float GetModCritMul()
        {
            return activeEffectMod.stats.critMultiplier + PerkManager.GetUnitModCritM(prefabID) +
                   UnitManager.GetAuraCritMod(this, node);
        }

        public float GetCDmgMul()
        {
            return stats.cDmgMultip * GetMulCDmgMul() + GetModCDmgMul();
        }

        public float GetMulCDmgMul()
        {
            return activeEffectMul.stats.cDmgMultip * PerkManager.GetUnitMulCDmgMul(prefabID) *
                   UnitManager.GetAuraCDmgMul(this, node);
        }

        public float GetModCDmgMul()
        {
            return activeEffectMod.stats.cDmgMultip + PerkManager.GetUnitModCDmgMul(prefabID) +
                   UnitManager.GetAuraCDmgMod(this, node);
        }

        public float GetCHitPenalty()
        {
            return stats.cHitPenalty * GetMulCHitPen() + GetModCHitPen();
        }

        public float GetMulCHitPen()
        {
            return activeEffectMul.stats.cHitPenalty * PerkManager.GetUnitMulCHitPen(prefabID) *
                   UnitManager.GetAuraCHitPenaltyMul(this, node);
        }

        public float GetModCHitPen()
        {
            return activeEffectMod.stats.cHitPenalty + PerkManager.GetUnitModCHitPen(prefabID) +
                   UnitManager.GetAuraCHitPenaltyMod(this, node);
        }

        public float GetCCritPenalty()
        {
            return stats.cCritPenalty * GetMulCCritPen() + GetModCCritPen();
        }

        public float GetMulCCritPen()
        {
            return activeEffectMul.stats.cCritPenalty * PerkManager.GetUnitMulCCritPen(prefabID) *
                   UnitManager.GetAuraCCritPenaltyMul(this, node);
        }

        public float GetModCCritPen()
        {
            return activeEffectMod.stats.cCritPenalty + PerkManager.GetUnitModCCritPen(prefabID) +
                   UnitManager.GetAuraCCritPenaltyMod(this, node);
        }

        public float GetODmgMul()
        {
            return stats.oDmgMultip * GetMulODmgMul() + GetModODmgMul();
        }

        public float GetMulODmgMul()
        {
            return activeEffectMul.stats.oDmgMultip * PerkManager.GetUnitMulODmgMul(prefabID) *
                   UnitManager.GetAuraODmgMul(this, node);
        }

        public float GetModODmgMul()
        {
            return activeEffectMod.stats.oDmgMultip + PerkManager.GetUnitModODmgMul(prefabID) +
                   UnitManager.GetAuraODmgMod(this, node);
        }

        public float GetOHitPenalty()
        {
            return stats.oHitPenalty * GetMulOHitPen() + GetModOHitPen();
        }

        public float GetMulOHitPen()
        {
            return activeEffectMul.stats.oHitPenalty * PerkManager.GetUnitMulOHitPen(prefabID) *
                   UnitManager.GetAuraOHitPenaltyMul(this, node);
        }

        public float GetModOHitPen()
        {
            return activeEffectMod.stats.cHitPenalty + PerkManager.GetUnitModOHitPen(prefabID) +
                   UnitManager.GetAuraOHitPenaltyMod(this, node);
        }

        public float GetOCritPenalty()
        {
            return stats.oCritPenalty * GetMulOCritPen() + GetModOCritPen();
        }

        public float GetMulOCritPen()
        {
            return activeEffectMul.stats.oCritPenalty * PerkManager.GetUnitMulOCritPen(prefabID) *
                   UnitManager.GetAuraODmgMul(this, node);
        }

        public float GetModOCritPen()
        {
            return activeEffectMod.stats.oCritPenalty + PerkManager.GetUnitModOCritPen(prefabID) +
                   UnitManager.GetAuraODmgMod(this, node);
        }

        public int GetAttackRange()
        {
            return (int)Mathf.Round(stats.attackRange * GetMulARange() + GetModARange());
        }

        public float GetMulARange()
        {
            return activeEffectMul.stats.attackRange * PerkManager.GetUnitMulARange(prefabID) *
                   UnitManager.GetAuraAttackRangeMul(this, node);
        }

        public float GetModARange()
        {
            return activeEffectMod.stats.attackRange + PerkManager.GetUnitModARange(prefabID) +
                   UnitManager.GetAuraAttackRangeMod(this, node);
        }

        public int GetAttackRangeMin()
        {
            return (int)Mathf.Round(stats.attackRangeMin * GetMulARangeMin() + GetModARangeMin());
        }

        public float GetMulARangeMin()
        {
            return activeEffectMul.stats.attackRangeMin * PerkManager.GetUnitMulARangeMin(prefabID) *
                   UnitManager.GetAuraAttackRangeMinMul(this, node);
        }

        public float GetModARangeMin()
        {
            return activeEffectMod.stats.attackRangeMin + PerkManager.GetUnitModARangeMin(prefabID) +
                   UnitManager.GetAuraAttackRangeMinMod(this, node);
        }

        public int GetMoveRange()
        {
            return (int)Mathf.Round(stats.moveRange * GetMulMRange() + GetModMRange());
        }

        public float GetMulMRange()
        {
            return activeEffectMul.stats.moveRange * PerkManager.GetUnitMulMRange(prefabID) *
                   UnitManager.GetAuraMoveRangeMul(this, node);
        }

        public float GetModMRange()
        {
            return activeEffectMod.stats.moveRange + PerkManager.GetUnitModMRange(prefabID) +
                   UnitManager.GetAuraMoveRangeMod(this, node);
        }

        public float GetTurnPriority()
        {
            return stats.turnPriority * GetMulTPriority() + GetModTPriority();
        }

        public float GetMulTPriority()
        {
            return activeEffectMul.stats.turnPriority * PerkManager.GetUnitMulTPrioity(prefabID) *
                   UnitManager.GetAuraTurnPriorityMul(this, node);
        }

        public float GetModTPriority()
        {
            return activeEffectMod.stats.turnPriority + PerkManager.GetUnitModTPrioity(prefabID) +
                   UnitManager.GetAuraTurnPriorityMod(this, node);
        }

        public int GetSight()
        {
            return (int)Mathf.Round(stats.sight * GetMulSight() + GetModSight());
        }

        public float GetMulSight()
        {
            return activeEffectMul.stats.sight * PerkManager.GetUnitMulSight(prefabID) *
                   UnitManager.GetAuraSightMul(this, node);
        }

        public float GetModSight()
        {
            return activeEffectMod.stats.sight + PerkManager.GetUnitModSight(prefabID) +
                   UnitManager.GetAuraSightMod(this, node);
        }


        public bool IsStunned()
        {
            return activeEffectMod.stun;
        }

        public bool AbilityDisabled()
        {
            return activeEffectMod.disableAbility;
        }
        //public bool HasOverwatch(){ return activeEffectMod.overwatch; }

        public float GetAttackM()
        {
            return statsMelee.attack * GetAttackMul() + GetAttackMod();
        }

        public float GetHitM()
        {
            return statsMelee.hit * GetHitMul() + GetHitMod();
        }

        public float GetDmgHPMinM()
        {
            return statsMelee.dmgHPMin * GetMulDmgHPMin() + GetModDmgHPMin();
        }

        public float GetDmgHPMaxM()
        {
            return statsMelee.dmgHPMax * GetMulDmgHPMax() + GetModDmgHPMax();
        }

        public float GetDmgAPMinM()
        {
            return statsMelee.dmgAPMin * GetMulDmgAPMin() + GetModDmgAPMin();
        }

        public float GetDmgAPMaxM()
        {
            return statsMelee.dmgAPMax * GetMulDmgAPMax() + GetModDmgAPMax();
        }

        public float GetCritChanceM()
        {
            return statsMelee.critChance * GetMulCritChance() + GetModCritChance();
        }

        public float GetCritMultiplierM()
        {
            return statsMelee.critMultiplier * GetMulCritMul() + GetModCritMul();
        }

        public int GetAttackRangeMelee()
        {
        ...
        public void ApplyEffect(List<int> list)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                {
                    eff.activeVisualEffect =
                        ObjectPoolManager.Spawn(eff.activeVisualEffect, GetPos(), Quaternion.identity);
        ...
        {
            activeEffectMod = new Effect();
            activeEffectMod.stats.ResetAsModifier();
            activeEffectMul = new Effect();
            activeEffectMul.stats.ResetAsMultiplier();
        ...
        private float Rotate(Vector3 tgtPos)
        {
            //for move
        ...
        public bool Aiming(Vector3 tgtPoint)
        {
            ...
            return Quaternion.Angle(turretPivot.rotation, wantedRot) < 2 &
                   Quaternion.Angle(barrelPivot.rotation, wantedRotX) < 2;
        }
        ...
        IEnumerator ResetAim()
        {
            while (turretPivot != null)
            {
                turretPivot.localRotation =
                    Quaternion.Lerp(turretPivot.localRotation, defaultTurretRot, aimSpeed * Time.deltaTime);

                bool reset = false;

                if (barrelPivot != null)
                {
                    barrelPivot.localRotation = Quaternion.Lerp(barrelPivot.localRotation, defaultBarrelRot,
                        aimSpeed * Time.deltaTime * 2);
                    reset = (Quaternion.Angle(turretPivot.localRotation, defaultTurretRot) < 1 &
                             Quaternion.Angle(barrelPivot.localRotation, defaultBarrelRot) < 1);
                }
        ...
        {
            List<Node> path =
                AStar.SearchWalkableNode(node, targetNode, AStar.BypassUnitCode(this), canMovePastObs, true);
                    ...
                    while (true)
                    {
                        if (enableRotation)
                        {
                            ...
                            thisT.rotation = Quaternion.Slerp(thisT.rotation, wantedRot,
                                Time.deltaTime * moveSpeed * speedMul);
                        }
        ...
        public IEnumerator AttackRoutine(Node targetNode, bool isCounter = false, bool isOverwatch = false)
        {
            ...
            if (targetNode.unit != null)
            {
                ...
                if (GameControl.EnableCounterAttack() && !targetDestroyed && !isCounter && !isOverwatch &&
                    targetNode.unit.CanCounter(this))
                {
        ...
        public IEnumerator FireShootObject(GameObject soPrefab, Node tgtNode, bool aimAtUnit,
            Vector3 offset = default(Vector3))
        {
            ...
            {
                GameObject sObj =
                    (GameObject)Instantiate(soPrefab, shootPointList[i].position, shootPointList[i].rotation);
                ShootObject soInstance = sObj.GetComponent<ShootObject>();

                if (aimAtUnit && tgtNode.unit != null)
                {
                    if (i == shootPointList.Count - 1)
                        soInstance.InitShoot(tgtNode.unit, HitCallback, shootPointList[i]);
                    else soInstance.InitShoot(tgtNode.unit, null, shootPointList[i]);
                }
                else
                {
                    if (i == shootPointList.Count - 1)
                        soInstance.InitShoot(tgtNode, HitCallback, shootPointList[i], offset);
        ...
        public bool ApplyAttack(Ability srcAbility)
        {
            ...
            if (srcAbility.HasNegativeImpact())
            {
                ...
                {
                    Attack attack = new Attack(srcAbility, this);
                    Overlay(attack);
                    destroyed = _ApplyAttack(attack.damageHP, attack.damageAP);
                }
                else
                {
                    float dmg = srcAbility.GetRandHPModifier() *
                                DamageTable.GetMultiplier(srcAbility.damageType, armorType);
        ...
        {
            StartCoroutine(_Overlay(attack));
        }
        ...
        public IEnumerator _Overlay(Attack attack)
        {
            if (attack.hit)
            {
                ...
                {
                    if (attack.crit)
                        TBTK.TextOverlay("<i>Critical -" + Mathf.Round(attack.damageAP).ToString("f0") + "AP</i>",
                            GetPos() + new Vector3(0, .5f, 0)); //, new Color(0, .6f, .6f, 1));
                    else
                        TBTK.TextOverlay("<i>-" + Mathf.Round(attack.damageAP).ToString("f0") + "AP</i>",
                            GetPos() + new Vector3(0, .5f, 0)); //, new Color(.2f, 1f, 1f, 1));
                ...
                {
                    if (attack.crit)
                        TBTK.TextOverlay("Critical -" + Mathf.Round(attack.damageHP).ToString("f0") + "HP",
                            GetPos()); //, new Color(1f, .6f, .3f, 1));
                    else
                        TBTK.TextOverlay("-" + Mathf.Round(attack.damageHP).ToString("f0") + "HP",
                            GetPos()); //, new Color(1f, .5f, .5f, 1));
                }
            }
        ...
        [Header("Animation")] public Transform animatorT;
        protected Animator animator;

        [Space(5)] public AnimationClip clipIdle;
        ...
        private void InitAnimation()
        {
            ...
            AnimatorOverrideController aniOverrideController =
        ...
        */
        ...
        [Header("Audio")] public bool loopMoveSound = false;
        ...
        private void InitAudio()
        {
            ...
            audioSrc.loop = true;
            audioSrc.volume = 1; //src.spatialBlend=.75f;
