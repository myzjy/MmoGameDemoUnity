...
namespace TBTK
{
    public class UIInput : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (Input.GetMouseButtonDown(1) && UIControl.EnableUnitInfo() && node != null && node.unit != null &&
                !UnitManager.DeployingUnit())
            {
            ...
            if (!UIControl.InTouchMode() && newNode && node != null && !GameControl.ActionInProgress() &&
                UnitManager.GetSelectedUnit() != null)
            {
            ...
            if (Input.GetKeyDown(KeyCode.Escape))
            {
                if (AbilityManager.IsWaitingForTarget())
                {
                } //quit ability target mode is handled in UIAbility script
        ...
        public static void ClearTouchModeCursor()
        {
            instance._ClearTouchModeCursor();
        }
        ...
        public void SetRoute(Node node)
        {
            ...
            List<Node> path =
                AStar.SearchWalkableNode(sUnit.node, node, AStar.BypassUnitCode(sUnit), sUnit.canMovePastObs);
            path.Insert(0, sUnit.node);

            var pos = new Vector3[path.Count];
            for (var i = 0; i < path.Count; i++) pos[i] = path[i].GetPos() + new Vector3(0, 0.25f, 0);
        ...
        public void RouteIndicatorTextureScroll()
        {
            ...
            mat.SetTextureOffset(MainTex, uvOffset);
        }
    }
}