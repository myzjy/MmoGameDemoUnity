...
namespace ZJYFrameWork.AssetBundles.Bundles.LoaderBuilders
{
    ...
    public class FileAsyncBundleLoader : BundleLoader
    {
        ...
        public FileAsyncBundleLoader(Uri uri, BundleInfo bundleInfo, BundleManager manager) : base(uri, bundleInfo,
            manager)
        {
        }
        ...
        protected override IEnumerator DoLoadAssetBundle(IProgressPromise<float, AssetBundle> promise)
        {
            if (this.BundleInfo.IsEncrypted)
            {
                ...
                promise.SetException(new NotSupportedException(string.Format(
                    "The data of the AssetBundle named '{0}' is encrypted,use the CryptographBundleLoader to load,please.",
                    this.BundleInfo.Name)));
                yield break;
            }
