...
namespace TBTK
{
    public class AI : MonoBehaviour
    {
        ...
        //when true, unit starts in passive state, then switch to aggressive or evasive (doesnt apply for passive)
        ...
        public static bool IsPassive(Unit unit)
        {
            ...
            {
                return unit.IsPassive();
            }
            else
            {
                return instance.aiBehaviour == AI._AIBehaviour.passive;
            }
        }
        ...
        public static bool IsAggressive(Unit unit)
        {
            ...
            {
                return (instance.aiBehaviour == AI._AIBehaviour.aggressive && !instance.requireTrigger) ||
                       unit.triggered;
            }
        }
        ...
        [Tooltip(
            "How much the critical chance for a potential attack is going to be weight into making the AI decision ")]
        public float critChanceMultiplier = 1;

        [Tooltip(
            "How much of giving chase on out of range target is going to be weight into making the AI decision\nOnly applies there are no target within range")]
        public float pursueMultiplier = 1;

        [Tooltip(
            "How much cover is going to be weight into making the AI decision\nOnly applies when cover system is enabled ")]
        public float coverMultiplier = 1;

        public static float dmgMul()
        {
            return instance.damageMultiplier;
        }

        public static float hitMul()
        {
            return instance.hitChanceMultiplier;
        }

        public static float critMul()
        {
            return instance.critChanceMultiplier;
        }

        public static float pursueMul()
        {
            return instance.pursueMultiplier;
        }

        public static float coverMul()
        {
            return instance.coverMultiplier;
        }
        ...
        public static void Init()
        {
            if (instance == null)
            {
                instance = (AI)FindObjectOfType(typeof(AI));
            }
        }
        ...
        {
            instance.StartCoroutine(_MoveUnit(unit));
        }
        ...
        {
            instance.StartCoroutine(_MoveFaction(faction));
        }
        ...
        public static IEnumerator _MoveFaction(Faction faction)
        {
            ...
            for (int i = 0; i < unitList.Count; i++)
            {
                ...
                if (unit == null || (unit.hp <= 0 && unit.IsVisible()))
                {
                    yield return new WaitForSeconds(0.25f);
                }

                if (unit != null && instance.delayBetweenUnit > 0 && unit.IsVisible())
                    yield return new WaitForSeconds(instance.delayBetweenUnit);
            }
        ...
        public static AIAction AnalyseAction(Unit unit)
        {
            ...
            Node nearestHostileNode = null;
            float nearestHostileDist = Mathf.Infinity;
            //Node furthestHostileNode=null;	float furthestHostileDist=0;
            float maxNearestDistToHostile = 0;
            float minNearestDistToHostile = Mathf.Infinity;
            ...
            for (int i = 0; i < walkableList.Count; i++)
            {
                ...
                if (attackNodeList.Count > 0)
                {
                    ...
                    for (int n = 0; n < attackNodeList.Count; n++)
                    {
                        ...
                        action.score += (0.5f * (attack.damageHPMin + attack.damageHPMax)) / unitDamage * 100 *
                ...
                else
                {
                    ...
                    else if (IsAggressive(unit))
                    {
                        ...
                        if (!nearestHostileScanned)
                        {
                            ...
                            if (allhostileList == null)
                                allhostileList = UnitManager.GetAllHostileUnits(unit.GetFacID());
            ...
            if (!hasTargetWithinRange && unit.attackThisTurn <= 0)
            {
                ...
                if (IsAggressive(unit) && !GameControl.EnableCoverSystem())
                {
                    ...
                    List<Node> path = AStar.SearchWalkableNode(unit.node, nearestHostileNode,
                        AStar.BypassUnitCode(unit), unit.canMovePastObs, true);
                    if (path.Count > 0)
                    {
