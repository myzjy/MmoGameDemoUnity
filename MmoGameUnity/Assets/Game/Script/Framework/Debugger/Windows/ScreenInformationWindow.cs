...
namespace ZJYFrameWork.Debugger.Windows
{
    public sealed class ScreenInformationWindow : ScrollableDebuggerWindowBase
    {
        protected override void OnDrawScrollableWindow()
        {
            ...
            GUILayout.BeginVertical("box");
            {
                ...
                DrawItem("Screen Width",
                    StringUtils.Format("{} px / {} in / {} cm", Screen.width.ToString(),
                        ConverterUtils.GetInchesFromPixels(Screen.width).ToString("F2"),
                        ConverterUtils.GetCentimetersFromPixels(Screen.width).ToString("F2")));
                DrawItem("Screen Height",
                    StringUtils.Format("{} px / {} in / {} cm", Screen.height.ToString(),
                        ConverterUtils.GetInchesFromPixels(Screen.height).ToString("F2"),
                        ConverterUtils.GetCentimetersFromPixels(Screen.height).ToString("F2")));
        ...
        private string GetSleepTimeoutDescription(int sleepTimeout)
        {
            ...
            {
                SleepTimeout.NeverSleep => "Never Sleep",
                SleepTimeout.SystemSetting => "System Setting",
        ...
        {
            return StringUtils.Format("{} x {} @ {}Hz", resolution.width.ToString(), resolution.height.ToString(),
