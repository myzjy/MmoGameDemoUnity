...
namespace ZJYFrameWork.AssetBundles.Bundles
{
    public class CryptographBundleLoaderBuilder : AbstractLoaderBuilder
    {
        ...
        {
            return new CryptographBundleLoader(new Uri(this.BaseUri, bundleInfo.Filename), bundleInfo, manager,
                this.decryptor);
        }
    }
    ...
    public class CryptographBundleLoader : BundleLoader
    {
        ...
        public CryptographBundleLoader(Uri uri, BundleInfo bundleInfo, BundleManager manager,
            IStreamDecryptor decryptor) : base(uri, bundleInfo, manager)
        {
            this.decryptor = decryptor;
        }
        ...
        protected override IEnumerator DoLoadAssetBundle(IProgressPromise<float, AssetBundle> promise)
        {
            ...
            if (this.decryptor.AlgorithmName.Equals("AES128_CTR_NONE") &&
                !FileUtil.IsZipArchive(this.GetAbsolutePath()))
            {
        ...
        protected virtual IEnumerator DoLoadAssetBundleFromMemory(IProgressPromise<float, AssetBundle> promise)
        {
            if (!this.decryptor.AlgorithmName.Equals(this.BundleInfo.Encoding))
            {
                ...
                promise.SetException(new Exception(string.Format(
                    "The encryption algorithm '{0}' and decryption algorithm '{1}' does not match when decrypts Assetbundle {2}. ",
                    this.BundleInfo.Encoding, this.decryptor.AlgorithmName, this.BundleInfo.Name)));
                yield break;
            }
            ...
            using (UnityWebRequest www = new UnityWebRequest(path))
            {
                ...
                {
                    promise.SetException(new Exception(string.Format(
                        "Failed to load the AssetBundle '{0}' at the address '{1}'.Error:{2}", this.BundleInfo.Name,
                        path, www.error)));
                    yield break;
                }
            ...
            if (this.IsRemoteUri())
            {
                ...
                {
                    Debug.LogError("将AssetBundle“{}”保存到目录“{}”失败。原因:{}", this.BundleInfo.FullName, fullname, e);
                }
            }
            ...
            catch (Exception e)
            {
                promise.SetException(new Exception(
                    $"Failed to decrypt the AssetBundle '{this.BundleInfo.Name}' at the address '{path}'.Error:{e}"));
                yield break;
            }
            ...
            {
                promise.SetException(new Exception(
                    $"Failed to load the AssetBundle '{this.BundleInfo.Name}' at the address '{path}'."));
                yield break;
            }
        ...
        protected virtual IEnumerator DoLoadAssetBundleFromStream(IProgressPromise<float, AssetBundle> promise)
        {
            ...
            if (this.IsRemoteUri())
            {
                ...
                using (UnityWebRequest www = new UnityWebRequest(path))
                {
                    ...
                    {
                        promise.SetException(new Exception(
                            $"Failed to load the AssetBundle '{this.BundleInfo.Name}' at the address '{path}'.Error:{www.error}"));
                        yield break;
                    }
            ...
            catch (Exception e)
            {
                ...
                promise.SetException(new Exception(string.Format(
                    "Failed to decrypt the AssetBundle '{0}' at the address '{1}'.Error:{2}", this.BundleInfo.Name,
                    path, e)));
                yield break;
            }
            ...
            {
                promise.SetException(new Exception(string.Format(
                    "Failed to load the AssetBundle '{0}' at the address '{1}'.", this.BundleInfo.Name, path)));
                yield break;
            }
