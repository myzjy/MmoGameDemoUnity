...
_Time.__newindex = function(t, k, v)
    ...
    FrostLogE(string.format("Property or indexer `CS.UnityEngine.Time.%s' cannot be assigned to (it is read only)", k))
end
    ...
    if not _Time.is_start_synch_time then
        ...
        synch_time = function()
            ...
            local on_server_time_ack = function(server_time_info)
                ...
                local server_time = server_time_info.server_time + time_offset / 2
                Time:SetServerTime(server_time)
                FrostLogD('Cat:Time.lua[152] CS.UnityMMO.TimeEx.ServerTime', CS.UnityMMO.TimeEx.ServerTime, server_time)
