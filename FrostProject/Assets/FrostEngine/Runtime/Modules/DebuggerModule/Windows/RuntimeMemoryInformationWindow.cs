...
namespace FrostEngine
{
    public sealed partial class RuntimeMemoryInformationWindow<T> : ScrollableDebuggerWindowBase where T : Object
    {
        ...
        protected override void OnDrawScrollableWindow()
        {
            ...
            GUILayout.BeginVertical("box");
            {
                ...
                else
                {
                    ...
                    {
                        GUILayout.Label(StringUtils.Format(
                            "<b>{} {}s ({}) obtained at {}, while {} {}s ({}) might be duplicated.</b>",
                            samples.Count.ToString(), typeName, GetByteLengthString(sampleSize),
                            sampleTime.ToString("yyyy-MM-dd HH:mm:ss"), duplicateSimpleCount.ToString(),
                            GetByteLengthString(duplicateSampleSize)));
                    }
                    else
                    {
                        GUILayout.Label(StringUtils.Format("<b>{} {}s ({}) obtained at {}.</b>",
                            samples.Count.ToString(), typeName, GetByteLengthString(sampleSize),
                            sampleTime.ToString("yyyy-MM-dd HH:mm:ss")));
                    }
                    ...
                    for (int i = 0; i < samples.Count; i++)
                    {
                        GUILayout.BeginHorizontal();
                        {
                            GUILayout.Label(samples[i].Highlight
                                ? StringUtils.Format((string)"<color=yellow>{}</color>", (object)samples[i].Name)
                                : samples[i].Name);
                            GUILayout.Label(
                                samples[i].Highlight
                                    ? StringUtils.Format((string)"<color=yellow>{}</color>", (object)samples[i].Type)
                                    : samples[i].Type, GUILayout.Width(240f));
                            GUILayout.Label(
                                samples[i].Highlight
                                    ? StringUtils.Format("<color=yellow>{}</color>",
                                        GetByteLengthString(samples[i].Size))
                                    : GetByteLengthString(samples[i].Size), GUILayout.Width(80f));
                        }
                        GUILayout.EndHorizontal();
        ...
        private void TakeSample()
        {
            ...
            {
                long sampleSize = 0L;
                sampleSize = Profiler.GetRuntimeMemorySizeLong(t);
                this.sampleSize += sampleSize;
                this.samples.Add(new Sample(t.name, t.GetType().Name, sampleSize));
            }
            ...
            {
                if (this.samples[i].Name == this.samples[i - 1].Name &&
                    this.samples[i].Type == this.samples[i - 1].Type &&
                    this.samples[i].Size == this.samples[i - 1].Size)
                {
