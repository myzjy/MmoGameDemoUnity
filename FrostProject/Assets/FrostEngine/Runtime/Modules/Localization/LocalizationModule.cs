...
using FrostEngine.Localization;
...
namespace FrostEngine
{
    ...
    public sealed class LocalizationModule: Module, IResourceManager_Bundles
    {
        ...
        private void Start()
        {
            ...
            if (rootModule == null)
            {
                Debug.Fatal("Base component is invalid.");
                return;
            }
        ...
        private async UniTask<bool> AsyncInit()
        {
            ...
            {
                Debug.Fatal($"Must set defaultLanguage.");
                return false;
            }
#if UNITY_EDITOR
        ...
        public async UniTask LoadLanguageTotalAsset(string assetName)
        {
            ...
            if (!m_UseRuntimeModule)
            {
                Debug.LogWarning($"禁止在此模式下 动态加载语言");
                return;
            }
#endif
        ...
        public async UniTask LoadLanguage(string language, bool setCurrent = false, bool fromInit = false)
        {
            ...
            if (!m_UseRuntimeModule)
            {
                Debug.LogWarning($"禁止在此模式下 动态加载语言 {language}");
                return;
            }
#endif
            ...
            else
            {
                ...
                {
                    Debug.LogWarning($"请使用I2Localization.asset导出CSV创建内置多语言.");
                    return;
                }
            ...
            if (assetTextAsset == null)
            {
                Debug.LogWarning($"没有加载到目标语言资源 {language}");
                return;
            }

            Debug.Info($"加载语言成功 {language}");
        ...
        public bool SetLanguage(string language, bool load = false)
        {
            if (!CheckLanguage(language))
            {
                ...
                Debug.LogWarning($"当前没有这个语言无法切换到此语言 {language}");
                return false;
            }
        ...
        public bool SetLanguage(int languageId)
        {
            ...
            {
                Debug.LogWarning($"Error languageIndex. Could not set and check {languageId}  Language.Count = {m_AllLanguage.Count}.");
                return false;
            }
        ...
        public T LoadFromBundle<T>(string path) where T : Object
        {
            ...
            Debug.LogError($"Localization could not load {path}  assetsType :{typeof(T).Name}.");
            return null;
        }
    }
}