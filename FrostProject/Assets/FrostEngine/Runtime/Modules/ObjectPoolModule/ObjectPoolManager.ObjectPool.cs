    ...
    internal sealed partial class ObjectPoolManager : ModuleImp, IObjectPoolManager
    {
        ...
        private sealed class ObjectPool<T> : ObjectPoolBase, IObjectPool<T> where T : ObjectBase
        {
            ...
            public void Unspawn(object target)
            {
                ...
                else
                {
                    throw new GameFrameworkException(StringUtils.Format(
                        "Can not find target in object pool '{}', target type is '{}', target value is '{2}'.", new TypeNamePair(typeof(T), Name),
            ...
            public void SetLocked(object target, bool locked)
            {
                ...
                {
                    throw new GameFrameworkException(StringUtils.Format(
                        "Can not find target in object pool '{}', target type is '{}', target value is '{}'.", new TypeNamePair(typeof(T), Name),
                        target.GetType().FullName, target));
                }
            }
