    ...
    internal class SceneModuleImp : ModuleImp, ISceneModule
    {
            ...
            Action<float> progressCallBack = null)
        {
            if (sceneMode == LoadSceneMode.Additive)
            {
                ...
                {
                    Debug.LogWarning($"Could not load subScene while already loaded. Scene: {location}");
                    return subScene;
                }
            ...
            else
            {
                ...
                {
                    Debug.LogWarning($"Could not load MainScene while loading. CurrentMainScene: {_currentMainSceneName}.");
                    return null;
                }
        ...
        public bool ActivateScene(string location)
        {
            ...
            Debug.LogWarning($"IsMainScene invalid location:{location}");
            return false;
        }
        ...
        public UnloadSceneOperation UnloadAsync(string location)
        {
            ...
            if (subScene != null)
            {
                ...
                {
                    Debug.LogError($"Could not unload Scene while not loaded. Scene: {location}");
                    return null;
                }
                _subScenes.Remove(location);
                return subScene.UnloadAsync();
            }
            Debug.LogWarning($"UnloadAsync invalid location:{location}");
            return null;
        }
