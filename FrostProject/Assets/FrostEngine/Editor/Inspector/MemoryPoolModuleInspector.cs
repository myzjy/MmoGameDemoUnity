...
namespace FrostEngine.Editor.Inspector
{
    ...
    internal sealed class MemoryPoolModuleInspector : GameFrameworkInspector
    {
                ...
                foreach (KeyValuePair<string, List<MemoryPoolInfo>> assemblyMemoryPoolInfo in m_MemoryPoolInfos)
                {
                    ...
                    if (currentState)
                    {
                        EditorGUILayout.BeginVertical("box");
                        {
                            ...
                            {
                                string exportFileName = EditorUtility.SaveFilePanel("Export CSV Data", string.Empty, StringUtils.Format("Memory Pool Data - {}.csv", assemblyMemoryPoolInfo.Key), string.Empty);
                                if (!string.IsNullOrEmpty(exportFileName))
                                {
                                    try
                                    {
                                        ...
                                        foreach (MemoryPoolInfo memoryPoolInfo in assemblyMemoryPoolInfo.Value)
                                        {
                                            data[index++] = StringUtils.Format("{},{},{},{},{},{},{},{}", memoryPoolInfo.Type.Name, memoryPoolInfo.Type.FullName, memoryPoolInfo.UnusedMemoryCount.ToString(), memoryPoolInfo.UsingMemoryCount.ToString(), memoryPoolInfo.AcquireMemoryCount.ToString(), memoryPoolInfo.ReleaseMemoryCount.ToString(), memoryPoolInfo.AddMemoryCount.ToString(), memoryPoolInfo.RemoveMemoryCount.ToString());
                                        }

                                        File.WriteAllLines(exportFileName, data, Encoding.UTF8);
                                        Debug.Log(StringUtils.Format("Export memory pool CSV data to '{}' success.", exportFileName));
                                    }
                                    catch (Exception exception)
                                    {
                                        Debug.LogError(StringUtils.Format("Export memory pool CSV data to '{}' failure, exception is '{}'.", exportFileName, exception.ToString()));
                                    }
                                }
                            }
                        }
        ...
        {
            EditorGUILayout.LabelField(m_ShowFullClassName ? memoryPoolInfo.Type.FullName : memoryPoolInfo.Type.Name, StringUtils.Format("{}\t{}\t{}\t{}\t{}\t{}", memoryPoolInfo.UnusedMemoryCount.ToString(), memoryPoolInfo.UsingMemoryCount.ToString(), memoryPoolInfo.AcquireMemoryCount.ToString(), memoryPoolInfo.ReleaseMemoryCount.ToString(), memoryPoolInfo.AddMemoryCount.ToString(), memoryPoolInfo.RemoveMemoryCount.ToString()));
        }
