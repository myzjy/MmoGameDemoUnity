...
namespace GameMain
{
    public class ProcedureCreateDownloader : ProcedureBase
    {
        ...
        private async UniTaskVoid CreateDownloader()
        {
            ...
            {
                Debug.Info("Not found any download files !");
                ChangeState<ProcedureDownloadOver>(_procedureOwner);
            }
            else
            {
                //A total of 10 files were found that need to be downloaded
                Debug.Info($"Found total {_downloader.TotalDownloadCount} files that need download ！");
        ...
        {
            Debug.LogWarning("On RequestVersion");
            _curTryCount++;
            ...
            {
                Debug.LogError("LoadMgr.RequestVersion, remote url is empty or null");
            ...
            Debug.Info("RequestUpdateData, proxy:" + checkVersionUrl);

            try
            {
                HttpRequest request = new HttpRequest(new Uri(checkVersionUrl));
                request.Callback += (originalBhRequest, bhResponse) =>
                {
                    var updateDataStr = StringUtils.BytesToString(bhResponse.Data);
                    UpdateData updateData = JsonConvert.DeserializeObject<UpdateData>(updateDataStr);
        ...
        private void ShowUpdateType(UpdateData data)
        {
            ...
            else if (data.UpdateType == UpdateType.ResourceUpdate)
            {
                ...
                else if (data.UpdateStyle == UpdateStyle.Optional)
                {
                    ...
                    {
                        UILoadTip.ShowMessageBox(
                            string.Format(LoadText.Instance.Label_Load_Notice, $"{_totalSizeMb}MB"),
                            MessageShowType.TwoButton,
                            LoadStyle.StyleEnum.Style_StartUpdate_Notice,
                            StartDownFile, () => { ChangeState<ProcedureLoadAssembly>(_procedureOwner); });
                    }
                    //不提示
                    else if (data.UpdateNotice == UpdateNotice.NoNotice)
                    {
                ...
                {
                    Debug.LogError("LoadMgr._CheckUpdate, style is error,code:" + data.UpdateStyle);
                }
            }
        }
    }
