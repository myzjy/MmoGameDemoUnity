...
namespace GameMain
{
    public class ProcedureDownloadFile:ProcedureBase
    {
        ...
        private void OnDownloadProgressCallback(int totalDownloadCount, int currentDownloadCount, long totalDownloadBytes, long currentDownloadBytes)
        {
            ...
            string descriptionText = StringUtils.Format("正在更新，已更新{}，总更新{}，已更新大小{}，总更新大小{}，更新进度{}，当前网速{}/s", 
                currentDownloadCount.ToString(), 
                totalDownloadCount.ToString(), 
                StringUtils.GetByteLengthString(currentDownloadBytes), 
                StringUtils.GetByteLengthString(totalDownloadBytes), 
                GameModule.Resource.Downloader.Progress, 
                StringUtils.GetLengthString((int)CurrentSpeed));
            ...
            string updateProgress = StringUtils.Format("剩余时间 {0}({1}/s)", timeStr, StringUtils.GetLengthString((int)CurrentSpeed));
            Debug.Info(updateProgress);
        }
    }
}