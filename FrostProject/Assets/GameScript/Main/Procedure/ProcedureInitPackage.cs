        ...
        private async UniTaskVoid InitPackage(ProcedureOwner procedureOwner)
        {
            try
            {
                ...
                if (initializationOperation.Status == EOperationStatus.Succeed)
                {
                    ...
                    {
                        Debug.Info("Editor resource mode detected.");
                        ChangeState<ProcedurePreload>(procedureOwner);
                    }
                    // 单机模式。
                    else if (playMode == EPlayMode.OfflinePlayMode)
                    {
                        Debug.Info("Package resource mode detected.");
                             ...
                             playMode == EPlayMode.WebPlayMode)
                    {
                        ...
                        Debug.Info("Updatable resource mode detected.");
                        ChangeState<ProcedureUpdateVersion>(procedureOwner);
                    }
                    else
                    {
                        Debug.LogError("UnKnow resource mode detected Please check???");
                    }
                }
                else
                {
        ...
        private async UniTask<UpdateData> RequestUpdateData()
        {
            ...
            try
            {
                ...
                request.Callback += (originalBhRequest, bhResponse) =>
                {
                    ...
                    updateData = JsonConvert.DeserializeObject<UpdateData>(updateDataStr);
                };
                await request.Send();
                return updateData;
            }
