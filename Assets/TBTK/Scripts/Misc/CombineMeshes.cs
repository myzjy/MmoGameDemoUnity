...
public class CombineMeshes : MonoBehaviour
{
    ...
    public void Combine()
    {
        ...
        MeshRenderer[]
            rendererList =
                GetComponentsInChildren<MeshRenderer>(); //this will get the parent too, so start the loop at 1
        //获取格子的网格过滤器
        ...
        for (int i = 1; i < rendererList.Length; i++)
        {
            ...
            if (!TBTK.GridManager.UseIndividualCollider() || !rendererList[i].enabled)
                rendererList[i].gameObject.SetActive(false);
        }
        ...
        else
        {
            ...
            if (childT != null)
                transform.GetComponent<MeshRenderer>().material = childT.GetComponent<MeshRenderer>().material;
        }
    }
}