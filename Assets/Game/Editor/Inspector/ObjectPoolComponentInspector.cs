                    ...
                    if (objectInfos.Length > 0)
                    {
                        ...
                        foreach (ObjectInfo objectInfo in objectInfos)
                        {
                            EditorGUILayout.LabelField(
                                ...
                                StringUtils.Format("{}\t{}\t{}\t{}\t{}", objectInfo.Locked.ToString(),
                                    objectPool.AllowMultiSpawn
                        ...
                        if (GUILayout.Button("Export CSV Data"))
                        {
                            string exportFileName = EditorUtility.SaveFilePanel("Export CSV Data", string.Empty,
                                StringUtils.Format("Object Pool Data - {}.csv", objectPool.Name), string.Empty);
                            if (!string.IsNullOrEmpty(exportFileName))
                            {
                                try
                                {
                                    ...
                                    data[index++] = StringUtils.Format(
                                        "Name,Locked,{},Custom Can Release Flag,Priority,Last Use Time",
                                    ...
                                    Debug.Log(StringUtils.Format("Export object pool CSV data to '{}' success.",
                                        exportFileName));
                                }
                                catch (Exception exception)
                                {
                                    Debug.LogError(StringUtils.Format(
                                        "Export object pool CSV data to '{}' failure, exception is '{}'.",
