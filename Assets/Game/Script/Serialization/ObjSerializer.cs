//#define USE_STATIC_OBJ_SERIALIZER
//#define PROFILE_DYNAMIC_DESERIALIZATION
...
{
	// オブジェクトをDictionary, List等の単純なオブジェクトに変換します
	...
	public class ObjSerializer
	{
		...
		public class FieldEntry
		{
			...
			public bool permitNull { get { return true; /*optional != null && !optional.notNull;*/ } }
			public bool permitNoField { get { return true; /*optional != null;*/ } }
			public bool generateDefault { get { return false; /*optional != null && optional.generateDefault;*/ } }
		}
		...
		static PureObject SerializeImpl(SerializationContext context, object o, Type typeHint = null)
		{
			if (o == null)
			{
//				if(typeHint == typeof(XorLong) || typeHint == typeof(XorInt) || typeHint == typeof(XorFloat))
//				{
//					return 0;
//				}
//				else
...
//				}
			...
			{
				if (!((DateTime)o).IsValid())
				{
					return 0;
				}

				return (int)(((DateTime)o).ToUnixTimeStamp());
			}
			else if (o is TimeSpan)
			{
				return (int)(((TimeSpan)o).TotalSeconds);
			}
			else if (o is Vector2)
			{
				var vec = (Vector2)o;
				return new List<object>() {
					SerializeImpl(context, vec.x, typeof(float)), SerializeImpl(context, vec.y, typeof(float))
				};
			}
			...
			{
				return o is IObjSerializable ? (o as IObjSerializable).Serialize(context) : o;
			}
			else if (o is string)
			{
				return o;
			...
			else if (o.GetType().IsArray || o is IList)
			{
				...
				{
					list.Add(SerializeImpl(context, obj, elementType));
				}
		...
		public static object DeserializeEnum(SerializationContext context, Type t, PureObject o)
		{
			if (o is string s)
			{
				return EnumGetValueOrFirst(t, s);
			}
			else
			{
		...
		public static object DeserializeImpl(SerializationContext context, Type t, PureObject o)
		{
			lock (syncRoot)
			{
				...
				{
//					ExceptionDebugger.DetectUnsupportedException(ex);
					throw ex;
				}
			}
		}
			...
			foreach (var f in GetSerializeFields(actualClass))
			{
				...
				try
				{
					if (dic.TryGetValue(f.name, out obj))
					{
						...
						{
							throw new Exception(string.Format("{0}に{1}がセットされていません", t,
		...
		static List<FieldEntry> _GetSerializeFields(Type t)
		{
			var ret = t
				.GetFields( /*BindingFlags.NonPublic |*/
					BindingFlags.Public | BindingFlags.Instance | BindingFlags.FlattenHierarchy).Select(f =>
					new FieldEntry() { field = f, name = f.Name, deserializer = GetDeserializer(f.FieldType) })
				.ToList();
			ret.AddRange(
				from f in t.GetFields(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.FlattenHierarchy)
			...
			//上記のコメントアウト外したいけどpublic fieldにSerializableFieldAttributeを追加しないといけなくなるので別回し
