...
function UIPrefab:SetVisibility(inSlateVisibility)
    ...
    -- todo 向下通知 _prefabClassAsync 和 _child 隐藏事件
...
function UIPrefab:NotifyPrefabVisible(inVisibility)
    if not self:vGetPrefabType() then
        return
    end
...
function UIPrefab:Create(mediator, parentPrefabClass, parentTf, argument, customID, style, layer, asyncLoadID, bNotAddLayer, insertFirst)
    ...
    if not resourcePath then
        PrintError(self.__classname .. " \t UIPrefab.Create : resource path error")
        return
    end
...
function UIPrefab:Destroy()
    ...
    if #self._child > 0 then
        for i = 1, #self._child do
            if self._child[i] ~= null then
                PrintError("%s \t UIPrefab.Destroy : prefabClass %s \t %s \t %s \t %s", self.__classname, self._child[i].__classname, " of ", self.__classname, " is living")
            end
        end
    end

    if #self._prefabClassAsync > 0 then
        PrintError(self.__classname .. " UIPrefab.Destroy : prefabClassAsyn is not empty" .. self.__classname)
    ...
    AssetServices:Unload(self._uiAsset)
...
---@param insertFirst boolean 是否从头部插入节点
function UIPrefab:CreateChildPrefabClassAsyncCompleted(async, cls, parentTf, argument, customID, style, asyncLoadID, insertFirst)
    ...
    self:vOnCreateAsyncPrefabClassCompleted(prefabClass)
end
...
function UIPrefab:SetAsynIcon(image, assetName, bIsSetDefault)
    if not image or (not image:GetClass():IsChildOf(UE4.UImage) and not image:GetClass():IsChildOf(UE4.UCMProgressBar)) then
        PrintError(self.__classname, "UIPrefab.SetAsynIcon : image param error ")
        return
    end
    ...
    if not UE4Helper.IsValidPath(assetPath) then
        PrintError(self.__classname, "UIPrefab.SetAsynIcon : assetName param error ", assetPath)
        return
    end
...
function UIPrefab:GetPrefabClassByTag(tagName, param)
    if not param then
        PrintError(self.__classname, "GetPrefabClassByTag param is error, Prohibit modifying param in the process!", debug.traceback())
    end
    ...
    return self:_DoEachChildPrefab(function(inPrefabInstance)
        if inPrefabInstance and inPrefabInstance ~= null then
            -- PrintError("NewbieGuide UIPrefab ", self.__classname, inPrefabInstance.__classname, inPrefabInstance:GetCustomID(), tagName, tostring(param.CustomID))
...
--- 异步创建子UIPrefabClass（CreateAsyncChildPrefabClass）完成后回调
---@param prefabClass UIPrefab 异步创建的子prefabClass
---@return void
function UIPrefab:vOnCreateAsyncPrefabClassCompleted(prefabClass)
end
...
function UIPrefab:HasChildPrefab(inCustomID)
    ...
    if not tChildPrefab then
        return self:GetCreateAsyncChildPrefabClassStateByCustomID(inCustomID)
    end
...
-------------------------------------------------------------------------------------------
function UIPrefab:GetAsyncChildPrefabCount()
    return #self._prefabClassAsync
end
...
function UIPrefab:CreateChildPrefabClass(cls, parentTf, argument, customID, style, asyncLoadID, insertFirst)
    if not cls then
        PrintError("%s \t CreateChildPrefabClass failed, Can not found UIPrefab nil", self.__classname)
        return nil
    end

    if not parentTf then
        PrintError("%s \t CreateChildPrefabClass failed, Can not found parent nil", cls.__classname)
        return nil
    end
...
--- @param insertFirst boolean 是否从头部插入节点
--- @return void
function UIPrefab:CreateAsyncChildPrefabClass(cls, parentTf, argument, loadPriority, life, customID, style, insertFirst)
    if not cls or not parentTf then
        PrintError("%s \t UIPrefab.CreateAsyncChildPrefabClass : invalid parameter", self.__classname)
        return
    end
    if customID then
        if self:GetPrefabClassByCustomID(customID) then
            PrintError("%s \t UIPrefab.CreateAsyncChildPrefabClass : already exist prefab", self.__classname, customID)
            return
        end
        if self:GetCreateAsyncChildPrefabClassStateByCustomID(customID) then
            PrintError("%s \t UIPrefab.CreateAsyncChildPrefabClass : prefab is create", self.__classname, customID)
            return
        end
    end
    local obj = UIPrefabAsync()
...
--- @param onlyAsync boolean 只销毁符合条件的异步子UIPrefabClass，默认销毁所有符合条件的
--- @return void
function UIPrefab:DestroyChildPrefabClassByName(name, onlyAsync)
    if not name then
        PrintError("%s \t UIPrefab.DestroyChildPrefabClassByName : invalid parameter", self.__classname)
        return
    end
...
--- @param onlyAsync boolean 只销毁符合条件的异步子UIPrefabClass，默认销毁所有符合条件的
--- @return void
function UIPrefab:DestroyChildPrefabClassByCustomID(customID, onlyAsync)
    if not customID then
        PrintError("%s \t UIPrefab.DestroyChildPrefabClassByCustomID : invalid parameter", self.__classname)
        return
    end
...
--- 销毁所有子UIPrefabClass
--- @param onlyAsync boolean 只销毁异步子UIPrefabClass，默认销毁所有
function UIPrefab:DestroyAllChildPrefabClass(onlyAsync)
    if not onlyAsync then
        for i = 1, #self._child do
...
function UIPrefab:GetPrefabClassByName(name)
    if not name then
        PrintError("%s \t UIPrefab.GetPrefabClassByName : invalid parameter", self.__classname)
        return nil
    end
...
function UIPrefab:GetPrefabClassByCustomID(customID)
    if not customID then
        PrintError("%s \t UIPrefab.GetPrefabClassByCustomID : invalid parameter", self.__classname)
        return nil
    end
...
function UIPrefab:GetCreateAsynChildPrefabClassStateByName(name)
    if not name then
        PrintError("%s \t UIPrefab.GetCreateAsynChildPrefabClassStateByName : invalid parameter", self.__classname)
        return
    end
...
--- @param customID string|number 自定义ID
--- @return boolean true：异步创建中  false: 不在异步创建中
function UIPrefab:GetCreateAsyncChildPrefabClassStateByCustomID(customID)
    if not customID then
        PrintError("%s \t UIPrefab.GetCreateAsyncChildPrefabClassStateByCustomID : invalid parameter", self.__classname)
        return
    end
    for i = #self._prefabClassAsync, 1, -1 do
        local pca = self._prefabClassAsync[i]
...
function UIPrefab:GetAutoLoadWidget(name, inIgnoreError)
    ...
    if not widget and not inIgnoreError then
        PrintError("%s \t GetAutoLoadWidget %s is not auto load!", self.__classname, name)
    end
    return widget
end
...
function UIPrefab:NotifyPreDestroyDelegate()
    if self._preDestroyDelegates and #self._preDestroyDelegates > 0 then
        for i = #self._preDestroyDelegates, 1, -1 do
            --存在调用中移除
            local preDestroyDelegate = self._preDestroyDelegates[i]
