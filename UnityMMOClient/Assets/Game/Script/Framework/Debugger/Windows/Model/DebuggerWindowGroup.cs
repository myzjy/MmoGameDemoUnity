...
namespace ZJYFrameWork.Debugger.Widows.Model
{
    public sealed class DebuggerWindowGroup : IDebuggerWindowGroup
    {
        private readonly List<KeyValuePair<string, IDebuggerWindow>> m_DebuggerWindows =
            new List<KeyValuePair<string, IDebuggerWindow>>();

        private int _mSelectIndex;

        private string[] _mDebuggerWindowsNames;
        ...
        public DebuggerWindowGroup()
        {
            m_DebuggerWindows =
                new List<KeyValuePair<string, IDebuggerWindow>>();
            _mSelectIndex = 0;
            _mDebuggerWindowsNames = null;
        }

        /// <summary>
        /// 获取调试器窗口数量。
        /// </summary>
        public int DebuggerWindowsCount
        {
            get => m_DebuggerWindows.Count;
        }
        ...
        public IDebuggerWindow SelectedWindow
        {
            get
            {
                if (_mSelectIndex >= m_DebuggerWindows.Count)
                {
                    return null;
                }

                return m_DebuggerWindows[_mSelectIndex].Value;
            }
        }
        ...
        {
            int index = 0;
            _mDebuggerWindowsNames = new string[m_DebuggerWindows.Count];
            foreach (var item in m_DebuggerWindows)
            {
                _mDebuggerWindowsNames[index++] = item.Key;
            }
        }
        /// <summary>
        /// 选中调试器窗口。
        ...
        {
            return (from debuggerWindow in m_DebuggerWindows
                where debuggerWindow.Key == name
                select debuggerWindow.Value).FirstOrDefault();
        }
