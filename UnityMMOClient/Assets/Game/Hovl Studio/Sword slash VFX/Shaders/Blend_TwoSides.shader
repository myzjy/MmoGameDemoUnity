Shader "ERB/Particles/Blend_TwoSides"
{
    ...
    Category
    {
        SubShader
        {
            Tags
            {
                "RenderType" = "Transparent" "Queue" = "Transparent" "IsEmissive" = "true" "PreviewType"="Plane"
            }
            Cull Off
        ...
        //    #include "UnityShaderVariables.cginc"
            ...
            void surf(Input i, inout SurfaceOutput o)
            {
                ...
                float4 staticSwitch101 = lerp(_FrontFacesColor,
                                                          ((_FrontFacesColor * (1.0 - fresnelNode95)) + (
                                                              _FresnelEmission
                                                              * _FresnelColor * fresnelNode95)), _UseFresnel);
                float dotResult87 = dot(ase_worldNormal, i.viewDir);
                float4 lerpResult91 = lerp(staticSwitch101, _BackFacesColor,
                                                              (1.0 + (sign(dotResult87) - -1.0) * (0.0 - 1.0) / (1.0 - -
                                                                  1.0)));
                float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
                float2 appendResult21 = (float2(_SpeedMainTexUVNoiseZW.x, _SpeedMainTexUVNoiseZW.y));
                o.Emission = (lerpResult91 * 2 * i.vertexColor * tex2D(
                    _MainTex, (uv_MainTex + (appendResult21 * _Time.y))) * i.vertexColor.a).rgb;
                o.Alpha = 1;
                ...
                clip((tex2D(_Mask, uv_Mask) * tex2D(_Noise, ((uv_Noise).xy + (_Time.y * appendResult22) + uv_Noise.w)) *
                    staticSwitch103).r - _Cutoff);
            }
            ENDCG
        }
    }
