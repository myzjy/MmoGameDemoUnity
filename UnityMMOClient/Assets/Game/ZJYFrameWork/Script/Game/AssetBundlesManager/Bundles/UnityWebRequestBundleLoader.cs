...
namespace ZJYFrameWork.AssetBundles.Bundles.LoaderBuilders
{
    public class UnityWebRequestBundleLoaderBuilder : AbstractLoaderBuilder
    {
        ...
        {
            return new UnityWebRequestBundleLoader(new System.Uri(this.BaseUri, bundleInfo.Filename), bundleInfo,
                manager, this.useCache);
        }
    }
    ...
    public sealed class UnityWebRequestBundleLoader : BundleLoader
    {
        ...
        public UnityWebRequestBundleLoader(System.Uri uri, BundleInfo bundleInfo, BundleManager manager,
            bool useCache = true) : base(uri, bundleInfo, manager)
        {
        ...
        protected override IEnumerator DoLoadAssetBundle(IProgressPromise<float, AssetBundle> promise)
        {
            if (this.BundleInfo.IsEncrypted)
            {
                ...
                promise.SetException(new NotSupportedException(
                    $"The data of the AssetBundle named '{this.BundleInfo.Name}' is encrypted,use the CryptographBundleLoader to load,please."));
                yield break;
            }
            ...
            {
#if UNITY_2018_1_OR_NEWER
                using (UnityWebRequest www = useCache
                           ? UnityWebRequestAssetBundle.GetAssetBundle(path, this.BundleInfo.Hash, 0)
                           : UnityWebRequestAssetBundle.GetAssetBundle(path))
                {
                    www.SendWebRequest();
#else
                using (UnityWebRequest www =
 useCache ? UnityWebRequest.GetAssetBundle(path, this.BundleInfo.Hash, 0) : UnityWebRequest.GetAssetBundle(path))
                {
                    www.Send();
#endif
                    while (!www.isDone)
