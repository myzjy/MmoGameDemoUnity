...
namespace AmplifyAnimationPack
{
    public class JumpState : CharacterStateBase
    {
        ...
        public override void EnterState()
        {
            ...
            currentJumpFowardVel = charRef.currLocomotionType == CharacterLocomotionTypes.walking
                ? charRef.jumpForwardVelocity * 0.75f
                : charRef.jumpForwardVelocity;
        }
        ...
        {
            if (Physics.Raycast(new Ray(charRef.hangPosHelper.position, charRef.transf.forward),
                    out wallpointDetectionHit, 1f))
            {
            ...
            if (charRef.rigid.velocity.y < 0)
            {
                ...
                charRef.rigid.velocity += (Vector3.up * Physics.gravity.y + charRef.transf.forward *
                                              currentJumpFowardVel * (charRef.isJumpingForward ? 1f : 0f))
            ...
            if (canLand)
            {
                var position = charRef.transf.position;
                UnityEngine.Debug.DrawLine(position, position - Vector3.up * 1.6f, Color.blue);
